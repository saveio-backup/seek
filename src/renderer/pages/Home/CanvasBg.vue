...
export default {
	data() {
		return {
			...
			minRadius: 2,
			maxRadius: 50,
			theme: ipcRenderer.sendSync("getSettings", "themeColor"),
			canvas: null,
			canvasTheme: {
				dark: {
					contextStrokeStyle: "rgba(100,100,100,0.2)",
					contextFillStyle: "rgba(100,100,100,0.1)",
					drawlineStrokeStyle: "200,200,200"
				},
				light: {
					contextStrokeStyle: "rgba(0,0,0,0.2)",
					contextFillStyle: "rgba(0,0,0,0.1)",
					drawlineStrokeStyle: "0,0,0"
				}
			},
			context: null,
			circleArr: [],
	...
	methods: {
		...
		initAttr() {
			...
			this.context.strokeStyle = this.canvasTheme[
				this.theme
			].contextStrokeStyle;
			this.context.strokeWidth = 1;
			this.context.fillStyle = this.canvasTheme[this.theme].contextFillStyle;
			this.circleArr = [];
		},
		...
		drawLine(cxt, x, y, _x, _y, o) {
			...
			cxt.strokeStyle = `rgba(${
				this.canvasTheme[this.theme].drawlineStrokeStyle
			}, ${o})`;
			...
			for (var i = 0; i < this.point; i++) {
				this.circleArr.push(
					this.drawCricle(
						...
						this.num(this.maxRadius, this.minRadius),
						this.num(10, -10) / 40,
						this.num(10, -10) / 40
					)
				);
